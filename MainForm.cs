...
namespace AutoDrop
{
    #region MÔ TẢ CHUNG VỀ ỨNG DỤNG
    public partial class MainForm : Form
    {
        #region KHAI BÁO CÁC BIẾN VÀ HÀM
        ...
        private const int DEFAULT_PASTE_SPEED = 200; // Tốc độ dán mặc định (250ms)
        ...
        #region HÀM KHỞI TẠO VÀ SỰ KIỆN
        // Hàm khởi tạo form
        public MainForm() // Hàm khởi tạo form
        {
            InitializeComponent(); // Khởi tạo các thành phần của form
            // Khởi tạo công cụ dán
            InitializeAutoDrop(); // Khởi tạo công cụ dán
            // Khởi tạo đồng hồ
        ...
        private void MainForm_Load(object sender, EventArgs e) // Sự kiện khi form được tải
        {
            btnSTART.Enabled = !string.IsNullOrWhiteSpace(txtTextbox.Text);
            ...
            UpdateManager.InitializeAutoCheck(
                exeName: "FAB Inspection.exe",
                httpServers: new[]
                {
                "http://phan:8888/update/DROP/",
                ...
                unlockBatBaseUrl: "http://107.126.41.111:8888/example/"  // ⭐ TRUYỀN VÀO
            );
        }       

        // Khi form đóng, hủy đăng ký phím nóng
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e) // Khi form đóng
        {
            ...
            this.Dispose(); // Giải phóng tài nguyên của form
            base.Dispose(); // Gọi phương thức Dispose của lớp cơ sở
        }
        #endregion

        #region HIỆU ỨNG CẦU VỒNG TÊN TÁC GIẢ
        ...
        #region CÔNG HÀM XỬ LÝ THỜI GIAN VÀ NÚT ẤN
        ...
        private void InitializeAutoDrop() // Khởi tạo các biến và trạng thái cho công cụ dán
        {
        ...
        private void BtnSTOP_Click(object sender, EventArgs e) // Sự kiện khi nhấn nút STOP
        {
            ...
            UpdateStartButtonState(); // Kích hoạt nút START
        ...
        private void ResetForm() // Đặt lại form về trạng thái ban đầu
        {
            ...
            InitializeAutoDrop(); // Khởi tạo lại công cụ dán
        ...
        #region XỬ LÝ PHÍM NÓNG VÀ DÁN VĂN BẢN
        ...
        {
            int pasteSpeed = DEFAULT_PASTE_SPEED;

            // --- Lấy tốc độ người dùng nhập, đặt ngưỡng an toàn ---
            if (!string.IsNullOrWhiteSpace(txtSpeed.Text))
            {
                if (!int.TryParse(txtSpeed.Text, out pasteSpeed))
                {
                    ...
                    MessageBox.Show(
                        "Tốc độ không hợp lệ. Sử dụng tốc độ mặc định: " + DEFAULT_PASTE_SPEED + "ms",
            ...
            // --- Ngăn tốc độ quá thấp gây SKIP dòng ---
            if (pasteSpeed < 120)
                pasteSpeed = 120;
            ...
            {
                MessageBox.Show("Vui lòng nhập dữ liệu trước khi bắt đầu.",
                    "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // --- Chuẩn hóa danh sách dòng, bỏ dòng trắng ẩn ---
            lines = txtTextbox.Text
            ...
            if (lines.Count == 0)
            {
                MessageBox.Show("Không có dòng hợp lệ để dán.",
                    "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                while (isPasting && currentLineIndex < lines.Count)
                {
                    ...
                    // --- Đặt Clipboard và đảm bảo nó đã thực sự cập nhật ---
                    Clipboard.SetText(text);
                    await Task.Delay(40);

                    int retry = 0;
                    while (Clipboard.GetText() != text && retry < 5)
                    {
                        await Task.Delay(20);
                        retry++;
                    }
                    ...
                    // --- Trễ an toàn tránh mất dòng ---
                    await Task.Delay(140);

                    // --- Xuống dòng ---
                    SendKeys.SendWait("{ENTER}");

                    // --- Tăng index & cập nhật giao diện ---
                    currentLineIndex++;
                    UpdateLineCountLabel();
                    UpdateFormTitle();

                    // --- Delay giữa mỗi lần dán ---
                    await Task.Delay(pasteSpeed);
                }
                ...
                StopPasting(isCompleted);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Đã xảy ra lỗi khi dán:\n" + ex.Message,
                    "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        #endregion

        #region CÁC HÀM TIỆN ÍCH KHÁC
        ...
        private void MainForm_Resize(object sender, EventArgs e) // Khi form thay đổi kích thước
        {
            UpdateScrollbarVisibility(); // Cập nhật thanh cuộn
        }
        #endregion
